 ...
 * server hos started so all the requests that comes to this application is forwarded to the
 * wiremock server via zuul.
 *
 * @author Divyaa P
 */
@Service
public class ZuulProxyService {
  ...
  @Autowired
  public ZuulProxyService(
      @Value("${wiremock.basePath}") String wiremockBasePath,
      @Value("${application.endpoint}") String applicationEndpoint,
  ...
  /**
   * Creates a zuul route with the following mapping - All requests to the endpoint
   * /wiremock/{application}/{environment}/** is forwarded to http://localhost:{port}
   *
  ...
  public void createZuulRoute(WiremockServerKey wiremockServerKey, int port) {
    String path =
        String.format(
            applicationEndpoint,
            wiremockServerKey.getApplication(),
            wiremockServerKey.getEnvironment());

    String forwardedToUrl = String.format(wiremockBasePath, port);

    zuulProperties
        .getRoutes()
        .put(UUID.randomUUID().toString(), new ZuulProperties.ZuulRoute(path, forwardedToUrl));
