...
public class WiremockInstanceProvider {
  ...
  @Autowired
  public WiremockInstanceProvider(
      ZuulProxyService zuulProxyService, WiremockStubMappingReader wiremockStubMappingReader) {
    this.zuulProxyService = zuulProxyService;
    this.wiremockStubMappingReader = wiremockStubMappingReader;
  }

  /**
   * Creates, configures, starts a wiremock server. Creates a zuul route for the new wiremock
   * server.
   *
   ...
   */
  private WireMockConfiguration configureWiremockServer(
      WiremockServerKey wiremockServerKey, int port) {
    WireMockConfiguration wireMockConfiguration =
        wireMockConfig()
            .port(port)
            .usingFilesUnderClasspath(
                wiremockStubMappingReader.getFilesClassPathRoot(wiremockServerKey))
